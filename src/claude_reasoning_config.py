...
class ClaudeReasoningConfig:
    ...
    def to_system_instruction(self) -> str:
        ...
        if self.reasoning_format:
            instructions.append(
                f"Format your reasoning as follows: {self.reasoning_format}"
            )
...
REASONING_TEMPLATES = {
    ...
    "direct": {"level": "none", "show_working": False},
    "extended_thinking": {
        "level": "medium",
...
def get_reasoning_config(template_name: str = None, **kwargs) -> ClaudeReasoningConfig:
    ...
    if template_name:
        if template_name not in REASONING_TEMPLATES:
            raise ValueError(
                f"Unknown reasoning template: {template_name}. "

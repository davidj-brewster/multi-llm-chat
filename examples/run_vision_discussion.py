...
"""
                    ...
                    logger.info(f"Multi-file conversation completed with {len(conversation)} messages")
                    
                elif config.input_file:
                    logger.info(f"Using single input file: {config.input_file.path} (Type: {config.input_file.type})")
        ...
        elif config.input_file and config.input_file.path and os.path.exists(config.input_file.path):
            logger.info(f"Processing single file for HTML output: {config.input_file.path}")
            try:
        ...
        if file_paths:
            ...
            if len(file_paths) == 1:
                ...
                output_file = f"multi_file_vision_discussion_{len(file_paths)}_files.html"
        else:

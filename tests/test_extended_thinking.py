...
async def main():
    ...
    if not api_key:
        print(
            "Error: ANTHROPIC_API_KEY not found in environment or ~/.ANTHROPIC_API_KEY file"
        )
        return

    # Get problem type and budget tokens
    problem_type = (
        sys.argv[1] if len(sys.argv) > 1 and sys.argv[1] in PROBLEMS else "math"
    )
    budget_tokens = (
        int(sys.argv[2]) if len(sys.argv) > 2 and sys.argv[2].isdigit() else 8000
    )
    ...
    response_basic = client1.generate_response(
        prompt=prompt, system_instruction=system_instruction, model_config=model_config
    )
    ...
    response_basic_str = (
        response_basic if isinstance(response_basic, str) else str(response_basic)
    )
    response_extended_str = (
        response_extended
        if isinstance(response_extended, str)
        else str(response_extended)
    )
    ...
    with open(extended_file, "w") as f:
        f.write(f"PROMPT: {prompt}\n\n")
        f.write(
            f"WITH EXTENDED THINKING (Budget: {budget_tokens}, Time: {extended_time:.2f}s):\n\n"
        )
    ...
    print(f"\n📊 Results Summary:")
    print(
        f"  - Basic response: {len(response_basic_str)} characters, {basic_time:.2f} seconds"
    )
    print(
        f"  - Extended thinking: {len(response_extended_str)} characters, {extended_time:.2f} seconds"
    )
    ...
    print("\nBASIC RESPONSE:")
    print(
        response_basic_str[:300] + "..."
        if len(response_basic_str) > 300
        else response_basic_str
    )
    print("\nEXTENDED THINKING RESPONSE:")
    print(
        response_extended_str[:300] + "..."
        if len(response_extended_str) > 300
        else response_extended_str
    )

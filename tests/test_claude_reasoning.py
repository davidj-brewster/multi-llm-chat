...
def main():
    ...
    template_name = (
        sys.argv[1]
        if len(sys.argv) > 1 and sys.argv[1] in REASONING_TEMPLATES
        else "step_by_step"
    )
    ...
    print(
        f"  Max reasoning tokens: {reasoning_config.max_reasoning_tokens or 'Unlimited'}"
    )
    ...
    if reasoning_config.extended_thinking:
        logger.info(
            f"Enabling extended thinking with budget_tokens={reasoning_config.budget_tokens}"
        )

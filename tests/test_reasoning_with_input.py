...
def get_sample_image_data():
    ...
    try:
        ...
        test_files = (
            list(Path(".").glob("T2-SAG-FLAIR.mov"))
            + list(Path(".").glob("*.jpg"))
            + list(Path(".").glob("*.png"))
        )
            ...
            with open(image_path, "rb") as f:
                ...
                if str(image_path).lower().endswith(".mov"):
                    ...
                    return {
                        ...
                        "mime_type": (
                            "image/jpeg"
                            if str(image_path).lower().endswith(".jpg")
                            else "image/png"
                        ),
                        "base64": base64.b64encode(image_data).decode("utf-8"),
                    }
    ...
    for level in reasoning_levels:
        try:
            ...
            response = client.generate_response(
                prompt=test_prompt, system_instruction="You are a programming expert."
            )

            # Check if we got a valid response
            if response and len(response) > 100:
                logger.info(
                    f"✅ Got valid response with reasoning_level='{level}' ({len(response)} chars)"
                )
                char_sample = response[:30].replace("\n", " ")
                logger.info(f'   Response start: "{char_sample}..."')
            else:
                logger.warning(
                    f"❌ Invalid or empty response with reasoning_level='{level}'"
                )
                return False
...
def test_claude_reasoning_with_multimodal():
    ...
    for level in reasoning_levels:
        try:
            ...
            if response and len(response) > 100:
                logger.info(
                    f"✅ Got valid multimodal response with reasoning_level='{level}' ({len(response)} chars)"
                )
                char_sample = response[:30].replace("\n", " ")
                logger.info(f'   Response start: "{char_sample}..."')
            else:
                logger.warning(
                    f"❌ Invalid or empty multimodal response with reasoning_level='{level}'"
                )
                return False

        except Exception as e:
            logger.error(
                f"Error testing Claude multimodal with reasoning_level='{level}': {e}"
            )
            return False

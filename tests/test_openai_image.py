...
def test_image_conversation(image_path):
    ...
    logger.info(f"Testing image conversation with responses API using image: {image_path}")
    ...
    try:
        ...
        system_instruction = "You are a helpful assistant with expertise in medical imaging."
...
if __name__ == "__main__":
    ...
    if not api_key:
        ...
        print("Turn 2 (no image): The model would remember the image and discuss seizure-related features")
        print("Turn 3 (no image): The model would continue analyzing hyperintensities in the remembered image")
        sys.exit(1)

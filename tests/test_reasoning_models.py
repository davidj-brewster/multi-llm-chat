...
def test_claude_client_direct():
    ...
    if not api_key:
        try:
            ...
            logger.error("Could not find ANTHROPIC_API_KEY in environment or ~/.ANTHROPIC_API_KEY file")
    ...
    for level_name, level_value in test_levels.items():
        try:
            ...
            logger.info(f"Setting reasoning_level to '{level_value}': client.reasoning_level = {client.reasoning_level}")
            assert client.reasoning_level == level_value, f"Expected reasoning_level to be {level_value}, got {client.reasoning_level}"
            
            # Verify capabilities 
            logger.info(f"Capabilities: {client.capabilities}")
            assert client.capabilities.get("advanced_reasoning") is True, "Claude 3.7 should have advanced_reasoning capability"
...
def test_openai_client_direct():
    ...
    if not api_key:
        try:
            ...
            logger.warning("Could not find OPENAI_API_KEY. Test will not make actual API calls.")
    ...
    for model_name in models_to_test:
        ...
        for level_name, level_value in test_levels.items():
            try:
                ...
                # Verify it was set correctly
                logger.info(f"Setting reasoning_level to '{level_value}' for {model_name}: client.reasoning_level = {client.reasoning_level}")
                assert client.reasoning_level == level_value, f"Expected reasoning_level to be {level_value}, got {client.reasoning_level}"
                
                # Verify capabilities 
                logger.info(f"Capabilities: {client.capabilities}")
                assert client.capabilities.get("advanced_reasoning") is True, f"OpenAI {model_name} should have advanced_reasoning capability"
                
            except Exception as e:
                logger.error(f"Error testing reasoning level '{level_value}' for {model_name}: {e}")
...
def test_conversation_manager():
    ...
    if not claude_api_key:
        try:
            ...
            logger.warning("Could not find ANTHROPIC_API_KEY in environment or ~/.ANTHROPIC_API_KEY file")
    ...
    if not openai_api_key:
        try:
            ...
            logger.warning("Could not find OPENAI_API_KEY in environment or ~/.OPENAI_API_KEY file")
    ...
    for model_name, expected_level in claude_test_models.items():
        try:
            ...
            logger.info(f"Model '{model_name}': client.reasoning_level = {client.reasoning_level}")
            assert client.reasoning_level == expected_level, f"Expected reasoning_level to be {expected_level} for model {model_name}, got {client.reasoning_level}"
            
            # Verify capabilities
            logger.info(f"Model '{model_name}' capabilities: {client.capabilities}")
            assert client.capabilities.get("advanced_reasoning") is True, f"Model {model_name} should have advanced_reasoning capability"

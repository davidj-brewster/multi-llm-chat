...
try:
    ...
    print(
        "Error: Could not import src package. Make sure you're running this script from the project root."
    )
    sys.exit(1)
...
def generate_module_doc(module_name: str, module: Any) -> None:
    ...
    for name, obj in inspect.getmembers(module):
        ...
        if inspect.isclass(obj) and obj.__module__ == module.__name__:
            classes.append(
                f"- [{name}]({name.lower()}.md): {extract_docstring(obj).split('.')[0]}"
            )
...
def generate_class_doc(class_name: str, cls: type, module_name: str) -> None:
    ...
    for name, obj in inspect.getmembers(cls):
        ...
        if isinstance(obj, property):
            ...
            if name != "__init__":
                methods.append(
                    f"- [{name}]({class_name.lower()}_{name.lower()}.md): {extract_docstring(obj).split('.')[0]}"
                )
                generate_method_doc(
                    f"{class_name}_{name}", obj, module_name, class_name
                )
    ...
    # Save markdown
    output_path = (
        OUTPUT_DIR / module_name.lower().replace(".", "/") / f"{class_name.lower()}.md"
    )
    save_markdown(content, output_path)


def generate_method_doc(
    method_name: str,
    method: callable,
    module_name: str,
